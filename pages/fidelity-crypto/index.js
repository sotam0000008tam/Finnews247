// pages/fidelity-crypto/index.js
import fs from "fs";
...
function Card({item}){const href=hrefFid(item.slug);const img=pickThumb(item);
...
export default function FidIndex({items=[],latest=[],page=1,totalPages=1}){
  const title="Fidelity Crypto";
  const canonical="https://www.finnews247.com/fidelity-crypto"+(page>1?`/page/${page}`:"");
  const description="Fidelity Crypto related posts.";
