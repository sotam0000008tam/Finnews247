...
const hrefBase = (slug) => `/crypto-market/${String(slug || "").replace(/^\//, "")}`;
...
export default function MarketPage({ items = [], latest = [], page = 1, totalPages = 1 }) {
  const title = "Crypto & Market";
  const canonical = "https://www.finnews247.com/crypto-market" + (page > 1 ? `/page/${page}` : "");
...
export async function getServerSideProps({ params }) {
  ...
  const posts = readCat("crypto-market");
  const PAGE_SIZE = 30;
