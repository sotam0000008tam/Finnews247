...
const hrefSec=(slug)=>`/sec-coin/${String(slug||"").replace(/^\//,"")}`;
const hrefMixed=(p)=>{if(p?.href)return p.href;const s=String(p?.slug||"").replace(/^\//,"");if(!s)return"#";const c=String(p?._cat||p?.category||"").toLowerCase();
  ...
  if(c.includes("altcoin"))return `/altcoins/${s}`; if(c.includes("fidelity"))return `/fidelity-crypto/${s}`;
  if(c.includes("exchange"))return `/crypto-exchanges/${s}`; if(c.includes("app")||c.includes("wallet"))return `/best-crypto-apps/${s}`;
  if(c.includes("insurance"))return `/insurance/${s}`; if(c.includes("tax")||c.includes("compliance"))return `/tax/${s}`;
  if(c.includes("guide")||c.includes("review"))return `/guides/${s}`; if(c.includes("market")||c.includes("news")||c.includes("crypto-market"))return `/crypto-market/${s}`;
  return `/guides/${s}`;};
...
export default function SecDetail({post,related=[],latest=[]}){
  ...
  const cname={base:"/sec-coin", title:"SEC Coin"};
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="grid md:grid-cols-12 gap-8">
        <article className="md:col-span-9">
          ...
          <div className="prose lg:prose-lg post-body" dangerouslySetInnerHTML={{__html:post.content||post.body||""}}/>
          ...
          <div className="mt-8">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {(related||[]).slice(0,9).map((it)=>(
                <Link key={it.slug} href={hrefSec(it.slug)} className="block rounded-lg border p-3 hover:bg-gray-50 dark:hover:bg-gray-800">
                  {/* Thumbnail */}
                  <img src={pickThumb(it)} alt={it.title} className="w-full h-40 object-cover rounded-md mb-3" loading="lazy"/>
                  {/* Date if available */}
