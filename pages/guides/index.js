...
const pickThumb = (p, fallback="/images/dummy/guides64.jpg") => p?.thumb || p?.ogImage || p?.image || firstImg(p?.content || p?.body || "") || fallback;

const FALLBACK_BASE = "/guides";
const buildUrl = (p) => {
...
export default function GuidesIndex({ posts = [], latest = [] }) {
  return (
    <>
      <NextSeo title="Guides & Reviews | FinNews247" canonical="https://www.finnews247.com/guides" />
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="grid md:grid-cols-12 gap-8">
          <section className="md:col-span-9 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {posts.map((it) => (
              <Link key={it.slug} href={buildUrl({ ...it, _cat: "guides" })} className="block rounded-lg border p-3 hover:bg-gray-50 dark:hover:bg-gray-800">
...
export async function getServerSideProps() {
  ...
  const posts = read("guides.json").filter(Boolean);
  ...
  const seen = new Set();
  const latest = [...market, ...alt, ...ex, ...apps, ...ins, ...tax]
