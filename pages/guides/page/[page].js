...
export default function GuidesPage({items=[],latest=[],page=1,totalPages=1}){
  const title="Guides & Reviews", canonical=`https://www.finnews247.com/guides/page/${page}`, description="Crypto guides & reviews.";
...
export async function getServerSideProps({params}){const read=(f)=>{try{return JSON.parse(fs.readFileSync(path.join(process.cwd(),"data",f),"utf-8"))}catch{return[]}};
  const posts=read("guides.json"); const PAGE_SIZE=30;
  const raw=parseInt(params.page,10)||1; const totalPages=Math.max(1,Math.ceil(posts.length/PAGE_SIZE));
  const page=Math.min(Math.max(1,raw),totalPages); const start=(page-1)*PAGE_SIZE;
  // sort trước rồi mới cắt trang
  const sorted = [...posts].sort((a,b) => (Date.parse(b.date || b.updatedAt) || 0) - (Date.parse(a.date || a.updatedAt) || 0));
