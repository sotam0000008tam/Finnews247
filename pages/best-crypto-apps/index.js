...
/* Helpers */
const stripHtml = (h = "") =>
  String(h).replace(/<script[\s\S]*?<\/script>/gi, "")
...
export default function AppsIndex({ items=[], latest=[], page=1, totalPages=1 }) {
  const title = "Crypto Apps & Wallets";
  const canonical = "https://www.finnews247.com/best-crypto-apps" + (page>1?`/page/${page}`:"");
          ...
          {totalPages>1 && (
            <div className="flex items-center justify-center gap-2 mt-6">
              {Array.from({length:totalPages}).map((_,i)=>{
                const p=i+1, href=p===1?"/best-crypto-apps":`/best-crypto-apps/page/${p}`, active=p===page;
