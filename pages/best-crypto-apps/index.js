...
const pickThumb = (p, fallback="/images/dummy/apps64.jpg") => p?.thumb || p?.ogImage || p?.image || firstImg(p?.content || p?.body || "") || fallback;
...
export default function AppsIndex({ posts = [], latest = [] }) {
  return (
    <>
      <NextSeo title="Apps & Wallets | FinNews247" canonical="https://www.finnews247.com/best-crypto-apps" />
      <div className="container mx-auto px-4 py-6">
        <h1 className="text-2xl md:text-3xl font-bold mb-4">Apps &amp; Wallets</h1>
...
export async function getServerSideProps() {
  ...
  const posts = read("bestapps.json").filter(Boolean);
