...
function Card({ item }){const href=hrefApps(item.slug);const img=pickThumb(item);return(<Link href={href} className="group block rounded-xl overflow-hidden border bg-white dark:bg-gray-900 hover:shadow-md transition">
...
export default function AppsPage({ items=[], latest=[], page=1, totalPages=1, signalsLatest=[] }){
  const title="Apps & Wallets";
  const canonical=page>1?`https://www.finnews247.com/best-crypto-apps/page/${page}`:"https://www.finnews247.com/best-crypto-apps";
