// pages/best-crypto-apps/page/[page].js
import fs from "fs";
...
const hrefMixed=(p)=>{if(p?.href)return p.href;const s=String(p?.slug||"").replace(/^\//,"");if(!s)return"#";const c=String(p?._cat||p?.category||"").toLowerCase();
  ...
  return `/guides/${s}`;};
function Card({item}){const href=hrefApps(item.slug);const img=pickThumb(item);return(
...
export default function AppsPage({items=[],latest=[],page=1,totalPages=1}){
  const title="Crypto Apps & Wallets"; const canonical=`https://www.finnews247.com/best-crypto-apps${page>1?`/page/${page}`:""}`; const description="Top crypto apps & wallets.";
...
export async function getServerSideProps({params}){
  ...
  const posts=read("bestapps.json"); const PAGE_SIZE=30;
