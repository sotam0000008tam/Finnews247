...
const pickThumb = (p) =>
  p?.thumb || p?.ogImage || p?.image || firstImg(p?.content || "") || "/images/dummy/apps64.jpg";

const BASE = "/best-crypto-apps";

export default function AppsPaged({ items = [], page = 1, totalPages = 1 }) {
  return (
...
export async function getServerSideProps({ params }) {
  ...
  const posts = read("bestapps.json").filter((p) => p?.slug);
