...
function guessAuthor(post){
  ...
  const raw=String(post.content||post.body||""); const m=raw.match(/(?:written\s+by|by)\s+([A-Z][\w .'-]{2,60})/i)||raw.match(/作者[:：]\s*([^\n<]+)/i);
...
export default function AppsDetail({post,related=[],latest=[]}){
  ...
  const cname={base:"/best-crypto-apps", title:"Crypto Apps & Wallets"};
          ...
          <div className="mt-8">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {(related||[]).slice(0,6).map((it)=>(
                <Link key={it.slug} href={hrefApps(it.slug)} className="block rounded-lg border p-3 hover:bg-gray-50 dark:hover:bg-gray-800">
...
export async function getServerSideProps({params}){
  ...
  const posts=read("bestapps.json");
  ...
  if(currentTags.length){
    relatedPool=relatedPool.map(p=>{
      ...
      const s=tags.filter(t=>tagSet.has(t)).length; const d=Date.parse(p.date||p.updatedAt)||0; return {p,s,d};
