...
/* ===== Helpers chung (giống Altcoin/Insurance) ===== */
const stripHtml = (h = "") =>
  String(h)
...
/* ===== Trading Signals (compact, KHÔNG thumbnail & KHÔNG Entry/Target/SL) ===== */
...
/* ===== PAGE ===== */
export default function AppsDetail({ post, related = [], latest = [], signalsLatest = [] }) {
  if (!post)
    return (
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="grid md:grid-cols-12 gap-8">
          ...
          <aside className="md:col-span-3 w-full sticky top-24 self-start space-y-6 sidebar-scope">
            {/* Trading Signals (compact, giống Altcoin/Insurance) */}
      ...
      {/* chỉ áp dụng cho block Latest (ảnh 45x45) */}
      <style jsx global>{`
        .sidebar-scope img {
...
export async function getServerSideProps({ params }) {
  ...
  const post =
    posts.find((p) => (p.slug || "").toLowerCase() === String(params.slug || "").toLowerCase()) ||
    null;
  if (!post) return { notFound: true };

  // Related: cùng category (sort mới→cũ), không scoring theo tag
  const relatedPool = posts
  ...
  // Latest: coverage mỗi chuyên mục + bù toàn site + sort mới→cũ
  const cats = [
    "crypto-market",
    "altcoins",
    "crypto-exchanges",
