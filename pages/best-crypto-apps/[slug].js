...
const pickThumb = (p) =>
  p?.thumb || p?.ogImage || p?.image || firstImg(p?.content || p?.body || "") || "/images/dummy/apps64.jpg";

/* URL builder: đọc _cat/category để định tuyến đúng */
const FALLBACK_BASE = "/best-crypto-apps";
const buildUrl = (p) => {
...
/* Guess author if data is missing */
function guessAuthor(post) {
  const direct =
    post.author || post.authorName || post.author_name || post.by || post.byline ||
    post?.meta?.author || post?.source?.author || "";
  if (direct && String(direct).trim()) return String(direct).trim();
  const raw = String(post.content || post.body || "");
  const m = raw.match(/(?:written\s+by|by)\s+([A-Z][\w .'-]{2,60})/i) || raw.match(/作者[:：]\s*([^\n<]+)/i);
  return m?.[1] ? m[1].trim().replace(/\s{2,}/g, " ") : "FinNews247 Team";
}
    ...
    <>
      ...
      <div className="container mx-auto px-4 py-6">
        <nav className="text-sm text-gray-500 mb-4">
          ...
          <Link href="/best-crypto-apps">Apps & Wallets</Link><span className="mx-2">/</span>
          ...
          <article className="md:col-span-9">
            ...
            <div className="mt-8">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-lg font-semibold">More from Apps & Wallets</h3>
                <Link href="/best-crypto-apps" className="text-sm text-sky-600 hover:underline">View all</Link>
              </div>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {(related || []).slice(0, 6).map((it) => (
                  <Link key={it.slug} href={buildUrl({ ...it, _cat: "best-crypto-apps" })} className="block rounded-lg border p-3 hover:bg-gray-50 dark:hover:bg-gray-800">
...
export async function getServerSideProps({ params }) {
  ...
  const pool = read("bestapps.json").filter(Boolean);
  ...
  const seen = new Set();
  const latest = poolLatest.filter((p) => { if (seen.has(p.slug)) return false; seen.add(p.slug); return true; })
