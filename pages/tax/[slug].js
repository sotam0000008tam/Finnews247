...
const hrefTax=(slug)=>`/tax/${String(slug||"").replace(/^\//,"")}`;
...
export default function TaxDetail({post,related=[],latest=[]}) {
  ...
  const cname={base:"/tax",title:"Crypto Tax"};
          ...
          <div className="mt-8">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {(related||[]).slice(0,9).map((it)=>(
                <Link key={it.slug} href={hrefTax(it.slug)} className="block rounded-lg border p-3 hover:bg-gray-50 dark:hover:bg-gray-800">
                  {/* Thumbnail */}
