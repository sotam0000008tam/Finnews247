...
export default function TaxIndex({ items = [], latest = [], page = 1, totalPages = 1 }) {
  const title = "Crypto Tax";
  const canonical = "https://www.finnews247.com/tax" + (page > 1 ? `/page/${page}` : "");
  const description = "Crypto tax and compliance topics.";
...
export async function getServerSideProps() {
  ...
  let pool = [];
  for (const c of cats) { try { pool = pool.concat(read(fileForCat(c)).map(p => ({ ...p, _cat: c }))); } catch {} }
  const seen = new Set();
