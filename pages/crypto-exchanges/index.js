...
const pickThumb = (p, fallback="/images/dummy/exchanges64.jpg") => p?.thumb || p?.ogImage || p?.image || firstImg(p?.content || p?.body || "") || fallback;

const FALLBACK_BASE = "/crypto-exchanges";
const buildUrl = (p) => {
...
export default function ExchangesIndex({ posts = [], latest = [] }) {
  return (
    <>
      <NextSeo title="Exchanges | FinNews247" canonical="https://www.finnews247.com/crypto-exchanges" />
      <div className="container mx-auto px-4 py-6">
        <h1 className="text-2xl md:text-3xl font-bold mb-4">Exchanges</h1>
        <div className="grid md:grid-cols-12 gap-8">
          <section className="md:col-span-9 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {posts.map((it) => (
              <Link key={it.slug} href={buildUrl({ ...it, _cat: "crypto-exchanges" })} className="block rounded-lg border p-3 hover:bg-gray-50 dark:hover:bg-gray-800">
...
export async function getServerSideProps() {
  ...
  const posts = [].concat(read("cryptoexchanges.json"), read("fidelity.json")).filter(Boolean);
