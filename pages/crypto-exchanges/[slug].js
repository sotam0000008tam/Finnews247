...
/* ===== Trading Signals (compact, KHÔNG Entry/Target/SL) ===== */
const prettyType = (t = "") =>
...
export async function getServerSideProps({ params }) {
  ...
  if (currentTags.length) {
    relatedPool = relatedPool
      .map((p) => {
        ...
        const s = tags.filter((t) => tagSet.has(t)).length;
  ...
  // latest mix (đảm bảo không trùng)
  const fileForCat = (c) =>
    (c === "crypto-exchanges"
      ? "cryptoexchanges"
      : c === "best-crypto-apps"
  ...
  for (const c of cats) {
    try {
      latestPool = latestPool.concat(
        (read(fileForCat(c)) || []).map((p) => ({ ...p, _cat: c }))
      );
    } catch {}
  }
  const seen = new Set();
