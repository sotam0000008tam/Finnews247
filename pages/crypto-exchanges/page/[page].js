...
function Card({ item }){const href=hrefEx(item.slug);const img=pickThumb(item);return(<Link href={href} className="group block rounded-xl overflow-hidden border bg-white dark:bg-gray-900 hover:shadow-md transition">
...
export default function ExchangesPage({ items=[], latest=[], page=1, totalPages=1, signalsLatest=[] }){
  const title="Crypto Exchanges";
  const canonical=page>1?`https://www.finnews247.com/crypto-exchanges/page/${page}`:"https://www.finnews247.com/crypto-exchanges";
    ...
    <div className="grid md:grid-cols-12 gap-8">
      <section className="md:col-span-9">
        ...
        {totalPages>1&&(<div className="flex items-center justify-center gap-2 mt-6">
          {Array.from({length:totalPages}).map((_,i)=>{const p=i+1;const href=p===1?"/crypto-exchanges":`/crypto-exchanges/page/${p}`;const active=p===page;
...
export async function getServerSideProps({ params }){
  ...
  // MAIN: gộp exchanges + fidelity
  const ex = readCat("crypto-exchanges")||[];
  const fi = readCat("fidelity")||[];
  const posts = uniqBySignature([...ex, ...fi]).sort((a,b)=>(Date.parse(b.date||b.updatedAt)||0)-(Date.parse(a.date||a.updatedAt)||0));
  ...
  const catLoad=(name)=>{if(name==="crypto-exchanges"){return uniqBySignature([...ex,...fi]);}
