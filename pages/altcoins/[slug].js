...
/* ===== Helpers (giống mẫu best-crypto-apps) ===== */
const stripHtml = (html = "") =>
  String(html).replace(/<script[\s\S]*?<\/script>/gi, "").replace(/<style[\s\S]*?<\/style>/gi, "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
...
const buildUrl = (p) => {
  ...
  const s = String(p?.slug || "").replace(/^\//, ""); if (!s) return "#";
...
export default function DetailPage({ post, related = [], latest = [], signalsLatest = [] }) {
  ...
  const description = (post.excerpt && stripHtml(post.excerpt)) || stripHtml(post.content || post.body || "").slice(0, 160);
  ...
  // 🔧 sort Latest mới → cũ (y như best-crypto-apps)
  ...
  return (
    <>
      <NextSeo title={pageTitle} description={description} canonical={canonical}
        openGraph={{ title: pageTitle, description, url: canonical, images: [{ url: ogImage }] }} />
      <div className="container mx-auto px-4 py-6">
        <nav className="text-sm text-gray-500 mb-4">
          <Link href="/">Home</Link><span className="mx-2">/</span>
          <Link href="/altcoins">Altcoin Analysis</Link><span className="mx-2">/</span>
        ...
        <div className="grid md:grid-cols-12 gap-8">
          <article className="md:col-span-9">
            ...
            <div className="mt-8">
              <div className="flex items-center justify-between mb-3">
                ...
                <Link href="/altcoins" className="text-sm text-sky-600 hover:underline">View all</Link>
              </div>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {(related || []).slice(0, 6).map((it) => (
                  <Link key={it.slug} href={`/altcoins/${it.slug}`} className="block rounded-lg border p-3 hover:bg-gray-50 dark:hover:bg-gray-800">
          ...
          <aside className="md:col-span-3 w-full sticky top-24 self-start space-y-6 sidebar-scope">
            ...
            <section className="rounded-xl border bg-white dark:bg-gray-900 overflow-hidden">
              ...
              <ul className="divide-y dark:divide-gray-800">
                {latestSorted.length ? latestSorted.map((it) => (
      ...
      <style jsx global>{`
        .sidebar-scope img{width:45px!important;height:45px!important;max-width:none!important;object-fit:cover!important;border-radius:8px!important;display:block!important;}
      `}</style>
    </>
  );
}
...
export async function getServerSideProps({ params }) {
  const read = (file) => {
    try { const p = path.join(process.cwd(), "data", file); return JSON.parse(fs.readFileSync(p, "utf-8")); }
    catch { return []; }
  };

  // own group: altcoins + seccoin
  const own = [].concat(read("altcoins.json"), read("seccoin.json")).filter(Boolean).flat();

  const post = own.find((p) => (p.slug || "").toLowerCase() === (params.slug || "").toLowerCase()) || null;
  ...
  for (const [cat, files] of Object.entries(groups)) {
    const arr = files.flatMap((f) => read(f) || []).map((p) => ({ ...p, _cat: cat }));
    arr.sort((a,b)=> (Date.parse(b.date || b.updatedAt) || 0) - (Date.parse(a.date || a.updatedAt) || 0));
    byCat[cat] = arr;
  }
  ...
  for (const cat of Object.keys(groups)) {
    ...
    if (pick) { seen.add(pick.slug); coverage.push(pick); }
  }
  const poolAll = Object.values(byCat).flat();
  const rest = poolAll.filter((p)=> p?.slug && !seen.has(p.slug))
  ...
  const latest = latestRaw.sort((a,b)=> (Date.parse(b.date || b.updatedAt) || 0) - (Date.parse(a.date || a.updatedAt) || 0));
