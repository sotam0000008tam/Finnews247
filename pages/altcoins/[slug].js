...
/* ===== Helpers (giữ nguyên từ bản của bạn) ===== */
...
/* ===== Page: Altcoins Post ===== */
export default function AltcoinsPostPage({ post, related = [], latest = [], signalsLatest = [] }) {
  if (!post)
    return (
  ...
  // Để ArticleSeo có canonical & breadcrumb chính xác (và xác định @type)
  const pathForSeo = `/altcoins/${post.slug}`;
  ...
  return (
    <>
      {/* ✅ SEO theo bài (OG/JSON-LD ảnh tuyệt đối; @type=Article cho altcoins) */}
