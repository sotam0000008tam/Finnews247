...
const hrefMixed=(p)=>{if(p?.href)return p.href;const s=String(p?.slug||"").replace(/^\//,"");if(!s)return"#";const c=String(p?._cat||p?.category||"").toLowerCase();
  ...
  if(c.includes("fidelity"))return `/fidelity-crypto/${s}`; if(c.includes("exchange"))return `/crypto-exchanges/${s}`;
  if(c.includes("app")||c.includes("wallet"))return `/best-crypto-apps/${s}`; if(c.includes("insurance"))return `/insurance/${s}`;
  if(c.includes("tax")||c.includes("compliance"))return `/tax/${s}`; if(c.includes("guide")||c.includes("review"))return `/guides/${s}`;
...
function SideMiniItem({item}){ const href=hrefMixed(item); const img=pickThumb(item);
  return(<Link href={href} className="group flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition">
...
export default function InsDetail({post,related=[],latest=[]}) {
  ...
  const cname={base:"/insurance", title:"Crypto Insurance"};
      ...
      <div className="grid md:grid-cols-12 gap-8">
        <article className="md:col-span-9">
          ...
          <div className="mt-2 mb-1 flex justify-end">
            <div className="flex items-center gap-2"><span className="text-[11px] uppercase tracking-wide text-gray-500">Written by:</span>
          ...
          <div className="mt-8">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {(related||[]).slice(0,6).map((it)=>(
                <Link key={it.slug} href={hrefIns(it.slug)} className="block rounded-lg border p-3 hover:bg-gray-50 dark:hover:bg-gray-800">
...
export async function getServerSideProps({params}) {
  ...
  const posts=read("insurance.json");
  const post=posts.find(p=>(p.slug||"").toLowerCase()===String(params.slug||"").toLowerCase())||null;
  if(!post) return {notFound:true};

  let relatedPool=posts.filter(p=>p.slug&&p.slug!==post.slug)
