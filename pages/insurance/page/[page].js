...
function Card({item}){ const href=hrefIns(item.slug); const img=pickThumb(item);
...
export default function InsPage({items=[],latest=[],page=1,totalPages=1}) {
  const title="Crypto Insurance", canonical=`https://www.finnews247.com/insurance${page>1?`/page/${page}`:""}`, description="Crypto insurance topics.";
...
export async function getServerSideProps({params}) {
  ...
  const posts=read("insurance.json"); const PAGE_SIZE=30;
