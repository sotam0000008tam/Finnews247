...
const hrefMixed=(p)=>{if(p?.href)return p.href;const s=String(p?.slug||"").replace(/^\//,"");if(!s)return"#";const c=String(p?._cat||p?.category||"").toLowerCase();
  ...
  return `/guides/${s}`;};
function Card({item}){const href=hrefIns(item.slug);const img=pickThumb(item);return(
...
export default function InsIndex({items=[],latest=[],page=1,totalPages=1}){
  const title="Crypto Insurance"; const canonical="https://www.finnews247.com/insurance"+(page>1?`/page/${page}`:""); const description="Crypto insurance topics.";
  return(<div className="container mx-auto px-4 py-6">
    ...
    <div className="grid md:grid-cols-12 gap-8">
      <section className="md:col-span-9"><div className="grid grid-cols-1 sm:grid-cols-2 gap-5">{items.map(it=>(<Card key={it.slug||it.title} item={it}/>))}</div>
...
export async function getServerSideProps(){const read=(f)=>{try{return JSON.parse(fs.readFileSync(path.join(process.cwd(),"data",f),"utf-8"))}catch{return[]}};
  const posts=read("insurance.json"); const PAGE_SIZE=30,page=1,totalPages=Math.max(1,Math.ceil(posts.length/PAGE_SIZE)); const items=posts.slice(0,PAGE_SIZE);
