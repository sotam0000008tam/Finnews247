...
export default function InsuranceIndex({
  ...
  const canonical = "https://www.finnews247.com/insurance";
  const description = "Crypto insurance & tax topics.";
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-6">
              {Array.from({ length: totalPages }).map((_, i) => {
                const p = i + 1;
                const href = p === 1 ? "/insurance" : `/insurance/page/${p}`;
                const active = p === page;
                return (
                  <Link
                    key={p}
...
export async function getServerSideProps() {
  ...
  // Insurance & Tax gộp
  const posts = uniqBySlug(readMany(["insurance", "tax"])).sort(
